...
// Route: /check-setup => Debug route to check database setup
app.get("/check-setup", (req, res) => {
	let status = {};

	connection.query("SHOW TABLES", (err, tables) => {
		if (err) {
			return res.json({ error: "Database not found or not connected", details: err.message });
		}
		...
		connection.query("SELECT * FROM users", (err, users) => {
			...
			connection.query("SELECT COUNT(*) as count FROM employees", (err, empCount) => {
				if (err) {
...
app.post("/api/employees", (req, res) => {
	...
	connection.query(
		...
		[employee_id, first_name, last_name, gender, date_of_birth, phone, email, position, deptId, employment_type, hire_date, salaryNum],
		(err, results) => {
			if (err) {
...
app.put("/api/employees/:id", (req, res) => {
	...
	connection.query(
		...
		[first_name, last_name, gender, phone, email, position, department_id, employment_type, salary, status, id],
		(err) => {
...
app.get("/api/reports/dashboard-stats", (req, res) => {
	...
	connection.query("SELECT COUNT(*) as count FROM employees WHERE status = 'active'", (err, results) => {
		stats.totalEmployees = results[0].count;

		connection.query("SELECT COUNT(*) as count FROM leave_requests WHERE status = 'pending'", (err, results) => {
			stats.pendingLeaveRequests = results[0].count;

			connection.query("SELECT COUNT(*) as count FROM attendance WHERE date = CURDATE() AND status = 'present'", (err, results) => {
				stats.todayPresent = results[0].count;

				connection.query("SELECT COUNT(*) as count FROM departments WHERE status = 'active'", (err, results) => {
					stats.totalDepartments = results[0].count;

					connection.query("SELECT employment_type, COUNT(*) as count FROM employees WHERE status = 'active' GROUP BY employment_type", (err, results) => {
